...
#pragma comment(lib, "ZCL")

#ifdef _DEBUG
    #pragma comment(lib, "strmbase_d")
#else
...
STDAPI RegisterFilters( BOOL bRegister )
{
    ...
    hr = CoInitialize(0);
    if(bRegister) {
        hr = AMovieSetupRegisterServer(CLSID_VirtualCam, PluginName, achFileName, L"Both", L"InprocServer32");
    }
    ...
    if( SUCCEEDED(hr) ) {
        ...
        if( SUCCEEDED(hr) ) {
            if(bRegister) {
                ...
                hr = fm->RegisterFilter(CLSID_VirtualCam, PluginName, &pMoniker, &CLSID_VideoInputDeviceCategory, NULL, &rf2);
            }
            else {
